...
_.extend(ObjectRenderer.prototype, NodeRenderer.prototype, {
  ...
  getW: function() {
    return this._getWFromCols(this.getText().length)
  },

  // Returns text to display on the object
  getText: function() { return (this.node.proto + ' ' + this.node.args.join(' ')).trim() },
...
_.extend(MultiLineObjectRenderer.prototype, ObjectRenderer.prototype, {
  ...
  renderTextWrapped: function(g, lines, myclass) {
    for(let n = 0; n < lines.length; ++n) {
      ...
      g.append('text')
        .attr('class', myclass)
        .text(lines[n])
        .attr('dy', this.getTextY()/2 + this.opts.glyphHeight/2 + this.lineHeight() * n)
  ...
  getH: function() {
    return Math.max(this.opts.objMinHeight,
      ...
      + this.lineHeight() * this.numRows - this.opts.lineSpacing // content
    )
  },

  getW: function() {
    return this._getWFromCols(this.numCols)
  },
...
_.extend(MsgRenderer.prototype, MultiLineObjectRenderer.prototype, {
  ...
  render: function(g) {
    let lines = getLines(this.getText())
  ...
  getText: function() { return this.node.args.join(' ').trim() },
...
_.extend(TextRenderer.prototype, MultiLineObjectRenderer.prototype, {
  render: function(g) {
    let lines = getLines(this.getText(), this.node.layout.width)
    this.setRowsCols(lines)
